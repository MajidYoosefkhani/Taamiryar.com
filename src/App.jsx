...
const Field = ({label,value,onChange,placeholder,multiline,type="text"}) => (
  <div style={{marginBottom:14}}>
    ...
    {multiline
      ? <textarea value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} rows={3} style={{width:"100%",background:"#0a0f1e",border:"1px solid rgba(56,189,248,.12)",borderRadius:10,color:"#e2e8f0",padding:"10px 13px",fontSize:13,fontFamily:"Tahoma,Arial,sans-serif",resize:"vertical",outline:"none",boxSizing:"border-box",transition:"all .2s",lineHeight:1.7}} onFocus={e=>{e.target.style.borderColor="rgba(56,189,248,.4)";e.target.style.boxShadow="0 0 0 3px rgba(56,189,248,.08)";}} onBlur={e=>{e.target.style.borderColor="rgba(56,189,248,.12)";e.target.style.boxShadow="none";}}/>
      : <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} style={{width:"100%",background:"#0a0f1e",border:"1px solid rgba(56,189,248,.12)",borderRadius:10,color:"#e2e8f0",padding:"10px 13px",fontSize:13,fontFamily:"Tahoma,Arial,sans-serif",outline:"none",boxSizing:"border-box",transition:"all .2s"}} onFocus={e=>{e.target.style.borderColor="rgba(56,189,248,.4)";e.target.style.boxShadow="0 0 0 3px rgba(56,189,248,.08)";}} onBlur={e=>{e.target.style.borderColor="rgba(56,189,248,.12)";e.target.style.boxShadow="none";}}/>
    }
  </div>
);
...
const Select = ({label,value,onChange,options}) => (
  <div style={{marginBottom:14}}>
    <label style={{display:"block",color:"#475569",fontSize:11,marginBottom:5,letterSpacing:1.2,textTransform:"uppercase"}}>{label}</label>
    <select value={value} onChange={e=>onChange(e.target.value)} style={{width:"100%",background:"#0a0f1e",border:"1px solid rgba(56,189,248,.12)",borderRadius:10,color:"#e2e8f0",padding:"10px 13px",fontSize:13,fontFamily:"Tahoma,Arial,sans-serif",outline:"none",boxSizing:"border-box",cursor:"pointer",appearance:"none"}}>
...
export default function Taamiryar() {
  ...
  const S = {
    ...
    btn:(active)=>({background:active?"linear-gradient(135deg,#0ea5e9,#6366f1)":"transparent",border:`1px solid ${active?"transparent":"rgba(56,189,248,.15)"}`,color:active?"#fff":"#64748b",borderRadius:10,padding:"10px 20px",fontFamily:"Tahoma,Arial,sans-serif",fontWeight:700,fontSize:13,cursor:"pointer",transition:"all .2s",boxShadow:active?"0 4px 20px rgba(14,165,233,.35)":"none"}),
  ...
  return (
    <div style={{minHeight:"100vh",background:"#030712",fontFamily:"Tahoma,Arial,sans-serif",direction:"rtl",color:"#f1f5f9",position:"relative",paddingBottom:80}}>
      <style>{`
      ...
      <header style={{position:"sticky",top:0,zIndex:50,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 20px",borderBottom:"1px solid rgba(56,189,248,.07)",backdropFilter:"blur(24px)",background:"rgba(3,7,18,.85)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontSize:18,fontWeight:900,fontFamily:"Georgia,serif",background:"linear-gradient(90deg,#38bdf8,#818cf8,#38bdf8)",backgroundSize:"200% auto",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",animation:"gradShift 4s linear infinite"}}>taamiryar</div>
      ...
      <nav style={{position:"fixed",bottom:0,left:0,right:0,zIndex:50,background:"rgba(3,7,18,.95)",backdropFilter:"blur(24px)",borderTop:"1px solid rgba(56,189,248,.08)",display:"flex",padding:"8px 0 12px"}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"4px 0",transition:"all .2s"}}>
            ...
            <span style={{fontSize:9,fontFamily:"Tahoma,Arial,sans-serif",color:tab===t.id?"#38bdf8":"#334155",fontWeight:tab===t.id?700:400,transition:"all .2s",letterSpacing:.5}}>{t.label}</span>
      ...
      <main style={{position:"relative",zIndex:5,maxWidth:680,margin:"0 auto",padding:"20px 16px 20px",opacity:mounted?1:0,transform:mounted?"none":"translateY(16px)",transition:"all .5s cubic-bezier(.22,1,.36,1)"}}>
        ...
        {tab==="home"&&(
          <div style={{animation:"fadeUp .5s ease"}}>
            <div style={{textAlign:"center",padding:"32px 0 28px"}}>
              ...
              <h1 style={{fontSize:28,fontWeight:900,fontFamily:"Georgia,serif",marginBottom:10,background:"linear-gradient(135deg,#f1f5f9 30%,#38bdf8 70%,#818cf8)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>taamiryar.com</h1>
        ...
        {tab==="diagnose"&&(
          <div style={{animation:"fadeUp .4s ease"}}>
            <div style={{marginBottom:20}}>
              <h2 style={{fontSize:22,fontWeight:900,fontFamily:"Georgia,serif",marginBottom:6,...S.gradText}}>تشخیص هوشمند</h2>
            ...
            {diagPhase==="idle"&&(
              <>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {SUGGESTIONS.map(s=>(
                    <button key={s} onClick={()=>{setQuery(s);setTimeout(()=>diagnose(s),50);}} style={{background:"transparent",border:"1px solid rgba(255,255,255,.05)",color:"#334155",borderRadius:12,padding:"5px 11px",fontSize:11,fontFamily:"Tahoma,Arial,sans-serif",cursor:"pointer",transition:"all .2s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(56,189,248,.25)";e.currentTarget.style.color="#38bdf8";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(255,255,255,.05)";e.currentTarget.style.color="#334155";}}>{s}</button>
                  ))}
                </div>
              </>
            )}
            {diagPhase==="loading"&&(
              <div style={{textAlign:"center",padding:"70px 0",animation:"fadeIn .3s ease"}}>
                ...
                <div style={{fontSize:16,fontWeight:700,fontFamily:"Georgia,serif",...S.gradText}}>در حال تشخیص{".".repeat(loadDots)}</div>
        ...
        {tab==="find"&&(
          <div style={{animation:"fadeUp .4s ease"}}>
            <div style={{marginBottom:20}}>
              <h2 style={{fontSize:22,fontWeight:900,fontFamily:"Georgia,serif",marginBottom:6,...S.gradText}}>یافتن تعمیرگاه</h2>
        ...
        {tab==="register"&&(
          <div style={{animation:"fadeUp .4s ease"}}>
            <div style={{marginBottom:20}}>
              <h2 style={{fontSize:22,fontWeight:900,fontFamily:"Georgia,serif",marginBottom:6,...S.gradText}}>ثبت تعمیرگاه</h2>
            ...
            {regStep===3?(
              <div style={{textAlign:"center",padding:"50px 20px",animation:"fadeUp .5s ease"}}>
                ...
                <div style={{fontSize:20,fontWeight:900,fontFamily:"Georgia,serif",marginBottom:8,...S.gradText}}>ثبت شد!</div>
      ...
      {showTip&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",backdropFilter:"blur(16px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20,animation:"fadeIn .2s ease"}} onClick={()=>setShowTip(false)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#070d1a",border:"1px solid rgba(56,189,248,.15)",borderRadius:24,padding:"32px 26px",width:"100%",maxWidth:360,textAlign:"center",boxShadow:"0 40px 80px #000",animation:"fadeUp .3s ease",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{fontSize:18,fontWeight:900,fontFamily:"Georgia,serif",marginBottom:8,...S.gradText}}>یه قهوه بریز!</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:18}}>
              {[{a:"$1",e:"☕",l:"قهوه"},{a:"$3",e:"🍕",l:"پیتزا"},{a:"$5",e:"🚀",l:"ویژه"},{a:"$10",e:"💎",l:"قهرمان"}].map(({a,e,l})=>(
                <button key={a} onClick={()=>setTipAmt(a)} style={{background:tipAmt===a?"linear-gradient(135deg,rgba(14,165,233,.2),rgba(99,102,241,.2))":"rgba(255,255,255,.02)",border:`1px solid ${tipAmt===a?"rgba(56,189,248,.4)":"rgba(255,255,255,.05)"}`,color:tipAmt===a?"#38bdf8":"#475569",borderRadius:12,padding:"12px 8px",fontFamily:"Tahoma,Arial,sans-serif",cursor:"pointer",transition:"all .2s"}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{if(tipAmt){setTipDone(true);setShowTip(false);}}} style={{flex:1,background:tipAmt?"linear-gradient(135deg,#0ea5e9,#6366f1)":"rgba(255,255,255,.03)",border:"none",borderRadius:12,padding:"13px",color:tipAmt?"#fff":"#1e3a4a",fontFamily:"Tahoma,Arial,sans-serif",fontWeight:700,fontSize:13,cursor:tipAmt?"pointer":"default",transition:"all .2s",boxShadow:tipAmt?"0 4px 20px rgba(14,165,233,.4)":"none"}}>
                {tipAmt?`💛 ${tipAmt} حمایت`:"مبلغ رو انتخاب کن"}
              </button>
              <button onClick={()=>setShowTip(false)} style={{background:"transparent",border:"1px solid rgba(255,255,255,.05)",color:"#1e3a4a",borderRadius:12,padding:"13px 14px",fontFamily:"Tahoma,Arial,sans-serif",fontSize:12,cursor:"pointer"}}>✕</button>
            </div>
          </div>
        </div>
      )}
